...
一部 UI 案では Markov 対角を `persistence` 閾値として表示するが、YoRuu では `MIN_PROB` と意味が重複するため、本設計では **案α（ローリング継続割合）** を採用する。Markov 対角の表示名は用語集の **Persistence**（分析用）と区別する (→ 第1章 1.6節)。

※ 実装上の Persistence 計算式は第11章 §11.4.1（および第10章 §10.9）を SSOT とする。本節「ローリング継続割合」は概念的説明であり、実装式は `min(P(UP→UP), P(DOWN→DOWN))`（α 案）に確定。
