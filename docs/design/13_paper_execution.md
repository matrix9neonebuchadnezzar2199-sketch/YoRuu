...
- [ ] §13.1 目的・スコープ明示（含む／含まない両方）
- [ ] §13.1.4 設計原則 5 項目明示
- [ ] §13.2 全体アーキテクチャ図 Mermaid 閉じる
- [ ] §13.2.3 Executor プロトコル定義
- [ ] §13.3 FillModel パラメータ 6 件揃う（既定値・範囲・用途）
- [ ] §13.3.4 スリッページ式と方向別適用ルール
- [ ] §13.3.5 遅延の PAPER / BACKTEST 差異明示
- [ ] §13.4 OpenRequest / CloseRequest / FillResult / OrderBook / ErrorPayload 全揃う
- [ ] §13.4.2 CloseReason 4 種類
- [ ] §13.5 BacktestExecutor との FillModel 共有
- [ ] §13.5.5 PAPER vs BACKTEST 差分表
- [ ] §13.6 LiveExecutor 対比表 9 項目以上
- [ ] §13.7.1 / §13.7.2 約定価格決定手順（open / close）
- [ ] §13.7.3 P&L 計算式と例
- [ ] §13.7.4 価格境界 [0.01, 0.99] クリップ
- [ ] §13.8 失敗ケース 5 種類 + リトライ表
- [ ] §13.9 保守的モデル原則と PHASE 5/6 検証手順
- [ ] §13.10 監査要件と `audit_log` 連携
- [ ] §13.11 相互参照表が新章番号で整合
- [ ] Mermaid コードフェンス全て閉じている

### 13.12.2 一次レビュー観点（7 項目）

1. PaperExecutor アーキテクチャ（§13.2）が `Executor` プロトコル準拠で `OrderManager` から透過的に呼べるか
2. FillModel パラメータ（§13.3.2）の既定値が「保守的」原則（§13.9.2）と整合し、PHASE 5/6 で再校正可能な粒度か
3. データ構造（§13.4）が第10章 DB スキーマ（§10.3.4 / §10.3.5）と完全整合するか
4. BacktestExecutor との FillModel 共有（§13.5）が再現性（決定論的）を担保し、PAPER との比較が公正か
5. LiveExecutor 対比表（§13.6）が第21章への伏線として十分か（プロトコル準拠・差分点が明確か）
6. 約定価格決定アルゴリズム（§13.7）と P&L 計算（§13.7.3）が Polymarket バイナリの仕様と整合するか
7. 失敗ケース分類（§13.8）と `wait_reason` / 状態遷移影響が第11章 §11.7 / 第3章と矛盾しないか
...
- §13.10.1 `trades.details_json` カラム追加（v1.0.4 候補、ch10 への追補）
