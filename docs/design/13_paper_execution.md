...
- `LiveExecutor` 概要と対比表（§13.6、詳細は第24章）
- 約定価格決定アルゴリズム（§13.7）
- 約定失敗ケース（§13.8）
...
- Polymarket CLOB クライアント実装詳細（→ 第24章）
- 戦略アルゴリズム（→ 第11章）
- 取引履歴 UI（→ 第8章 §8.13）
...
詳細は第24章（Polymarket CLOB クライアント）で確定。本章ではプロトコル準拠と対比のみ。暫定 API 契約は §10.7.7 `LiveExecutor` を参照。

### 13.6.3 LIVE 約定失敗時のフォールバック

LIVE では Polymarket API の失敗応答（流動性不足・タイムアウト・残高不足等）を `FillResult.success = False` で返す。リトライ戦略は第24章で規定（既定: 流動性不足は 1 回リトライ、残高不足はリトライなし）。
...
LIVE 固有エラー（タイムアウト・残高不足）は第24章で規定。
...
| 5 | LiveExecutor 対比・第24章伏線（§13.6） | ✅ 合格 |
