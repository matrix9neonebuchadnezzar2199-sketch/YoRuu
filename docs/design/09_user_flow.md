...
- **第9章 §9.6**: ダッシュボードでバナー受領 → 夜間レビュー画面遷移 → JSON コピー → 外部 AI で解析 → 結果ペースト → 差分確認 → Apply ボタン → 戦略履歴で確認 のユーザー操作
...
| ステップ | 画面 | 主要表示 |
|----------|------|----------|
| C | ブラウザ | localhost:8765 を開く |
| D | ハブ | 当日損益 $0.00、累積 $0.00、勝率 N/A、状態 IDLE |
| F-H | 設定（経由可） | yoruu.yaml の現在値表示 |
| G | ダッシュボード | サイドバーから遷移、状態 TRADING |
| K | モード切替 | 4モードカード、PAPER に [現在] バッジ |
| N | ダッシュボード（稼働確認） | 状態 TRADING、ポジション「待機中」 |
...
YoRuu の中核となる「夜間自己学習ループ」のユーザー操作部分。Bot が深夜にレポートを生成し、ユーザーが 外部 AI で解析、結果を Bot に返して戦略パラメータを更新する。

### 9.6.2 前提条件

- 当日の取引が一定数以上完了している（推奨: 20取引以上）
- `nightly_review.send_time` で設定した時刻を経過
- ユーザーが 外部 AI アカウントにログイン可能
...
| 必須キー不足 | 「MIN_PROB, MIN_EDGE, KELLY_FRACTION, PERSISTENCE_THRESHOLD のうち X が不足」 | 外部 AI で再質問 |
| 範囲外値 | 「MIN_PROB=0.75 は 0.80〜0.95 範囲外」、Apply 無効 | 外部 AI で再質問または手動修正 |
...
| 項目 | 対応方針 |
|------|----------|
| §9.6 AI 解析の所要時間 | 外部 AI の応答時間に依存。PHASE 6 で実測 |
...
- [x] §9.1 〜 §9.16 がすべて存在
- [x] 全10フローが定義され、それぞれ前提条件・手順・UI 状態・エラー分岐を含む
- [x] §9.6 夜間レビュー完全フローが他フローより詳細
- [x] §9.8 緊急停止と復帰の非対称設計が明示
- [x] §9.14 横断エラー分岐が定義されている
- [x] PHASE 2 / PHASE 4 引き継ぎが明示
- [x] 章間相互参照が新章番号と整合
