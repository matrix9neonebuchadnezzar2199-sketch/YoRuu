...
- [ ] §14.1 目的・スコープ明示（含む／含まない両方）
- [ ] §14.1.4 設計原則 5 項目明示
- [ ] §14.2 二言語ポリシー（`ja` 一次、`en` 空辞書）
- [ ] §14.2.2 既定言語決定順序 5 段階
- [ ] §14.3 命名規則（namespace.context.element.variant、最大 4 階層）
- [ ] §14.3.2 名前空間 11 種類全揃う
- [ ] §14.3.7 禁止事項（文末記号・動詞活用・番号付け）
- [ ] §14.4 辞書ファイル配置と JSON フラット構造
- [ ] §14.4.3 ネスト非採用の理由明示
- [ ] §14.5 フォールバック解決順序 3 段階
- [ ] §14.5.2 フォールバック時ログレベル表
- [ ] §14.6 UI 実装規約（`data-i18n` / 属性翻訳 / `t()` / 言語切替）
- [ ] §14.7 サーバー側 i18n（言語非依存原則）
- [ ] §14.7.2 エラーメッセージの英語フォールバック方針
- [ ] §14.7.5 alerts.message 日本語固定の判断明示
- [ ] §14.8 変数埋込・複数形・改行・HTML エスケープ
- [ ] §14.9 日時・数値・通貨・パーセントフォーマット
- [ ] §14.10 翻訳キー一覧（11 カテゴリ、代表例揃う）
- [ ] §14.10.7 `markov.*` が §8.20.5.1 と同期
- [ ] §14.11 開発時規約（追加・削除・CI バリデーション）
- [ ] §14.11.6 アクセシビリティ連携（`a11y.*`）
- [ ] §14.12 相互参照表が新章番号で整合

### 14.13.2 一次レビュー観点（7 項目）

1. 二言語ポリシー（§14.2）が「`ja` 一次 / `en` 空辞書」の現実的方針として明確か
2. 命名規則（§14.3）が 11 名前空間で UI 全要素を網羅できる粒度か
3. 辞書ファイル構造（§14.4）がフラット JSON で PHASE 2 モック ↔ PHASE 4 実装の SSOT 統一できるか
4. フォールバック規則（§14.5）が `en` 不在時に UI が破綻しない保証を与えるか
5. UI 実装規約（§14.6）が `data-i18n` 属性ベースで JS 無効時のフォールバック表示も成立するか
6. サーバー側 i18n（§14.7）の「言語非依存 API + クライアント解決」が §10.6.11 と整合するか
7. `markov.*`（§14.10.7）が §8.20.5.1 と §11.7 の wait_reason 6 値と完全同期するか
