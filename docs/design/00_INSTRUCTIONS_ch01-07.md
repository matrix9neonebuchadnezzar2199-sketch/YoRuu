...
**想定実行時間の記述ルール（必須）**:
- 測定前の数値は **必ず** `概算` / `測定前` / `目安` / `未測定` のいずれかを付ける
- 修飾語なしの具体秒数・ミリ秒は **禁止**（根拠と試算ロジックを併記した場合のみ可）
- 根拠例: 「Polymarket CLOB 公式ドキュメントの p99 レイテンシ &lt; 500ms より試算」
- 数値が書けない場合は `[要確認: 実測後に更新]` とし、憶測で埋めない
...
- 日次レポート JSON: **フィールド概要のみ**（トップレベルキー名と用途を1行ずつ）。完全スキーマは書かない `(→ 第15章で詳細)`
- 取引ログ CSV ヘッダ
- 戦略履歴 JSON

**スコープ制限**: 第15章の完全 JSON スキーマ定義に相当する記述は **禁止**。第15章本文の先走りはスコープ違反。
...
各章を生成後、`docs/design/INDEX.md` を更新すること。

**INDEX.md のルール**:
- **メイン目次**: 今回生成した7章のみ（章タイトル・ファイル名・1行サマリ・ステータス DRAFT/REVIEW_PENDING/APPROVED）
- **第8〜24章はメイン目次に載せない**（未着手章のタイトルだけ並べると、後の統合・分割時に混乱する）
- **任意**: 末尾に `<details>` で「今後の章（タイトルのみ）」を折りたたみ表示してよい（進捗の見える化用）

人間レビュー時は [`REVIEW_CHECKLIST_ch01-07.md`](REVIEW_CHECKLIST_ch01-07.md) を使用すること。
