...
- **承認日**: 2026-05-27
- **ステータス**: APPROVED
- **最終更新**: 2026-05-27（v1.0.2: CLOB 参照を ch13 / 未執筆章 TBD に統一、[`PHASE1_M13_MIDPOINT_REVIEW.md`](./PHASE1_M13_MIDPOINT_REVIEW.md)）
...
- LIVE 実取引の Polymarket CLOB クライアント実装詳細（→ 第13章 §13.6 / CLOB 章 TBD）
...
- [x] §12.1 目的・スコープ明示（含む／含まない両方）
- [x] §12.2 4 モード全てに用途・データソース・約定エンジン記述
- [x] §12.3 動作マトリクスに 16 項目以上の比較
- [x] §12.4 状態 × モード許可マトリクスが第3章 §3.1 と整合
- [x] §12.4.3 切替可能状態の明示（IDLE のみ + EMERGENCY_STOP 限定）
- [x] §12.5 切替フロー Mermaid が閉じている
- [x] §12.5.2.1〜12.5.2.3 モード切替 SSOT（状態ガード・HTTP・処理順序）が明示
- [x] §12.5.5 非 LIVE 切替（PAPER ↔ SIMMER）が明示
- [x] §12.5.3 EMERGENCY_STOP からの LIVE 直接復帰禁止明示
- [x] §12.6 LIVE 2 段階確認 7 項目全て揃う
- [x] §12.6.6 CLI `--confirm-live` フラグ仕様明示
- [x] §12.7 BACKTEST で `daily_loss_limit` 不適用の根拠明示
- [x] §12.7.3 LIVE の USDC 残高チェックと `reserved_buffer` 明示
- [x] §12.8 `trades.mode` でのデータ分離方針明示
- [x] §12.8.4 BACKTEST 格納先 3 オプション
- [x] §12.9 全遷移マトリクスに 4×4 全セル記載
- [x] §12.9.3 残高引継ぎルール 6 ケース
- [x] §12.10 起動・終了フロー（SIGTERM vs SIGKILL）
- [x] §12.11 相互参照表が新章番号で整合
