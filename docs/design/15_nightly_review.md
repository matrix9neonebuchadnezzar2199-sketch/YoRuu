...
| `warn_large_change: true`（変化率 ±10% 超） | 黄背景、`warnings[]` に `W_NIGHTLY_001`、Apply は **活性のまま**（ユーザー判断） |
| 変化率 ±20% 超 | 赤背景、`errors[]` に `E_NIGHTLY_008`、Apply 無効（強制承認パスは v1.1 で検討） |
...
±10% 超の警告（`W_NIGHTLY_001`）は **活性を阻害しない**。理由: 戦略チューニングの本質的な変更幅をブロックしないが、ユーザーに視覚的注意を促す。
...
- [ ] §15.1 目的・スコープ明示（含む 7 件 / 含まない 7 件）
- [ ] §15.1.4 設計原則 5 項目を箇条書き
- [ ] §15.2 全体フロー Mermaid 4 段階で描画
- [ ] §15.2.2 ch8 §8.14 / ch9 §9.6 / 本章の役割分担表
- [ ] §15.3 04:00 JST 既定、`NIGHTLY_REVIEW` 状態、ch3 との差異を `[要確認: ch3]` でフラグ
- [ ] §15.3.5 失敗時の最大 30 分リトライ規定
- [ ] §15.4 レポート JSON 7 セクション全て型・必須・サンプル付き
- [ ] §15.4.7 `constraints` が ch10 §10.4.1 / ch11 §11.10 と一致
- [ ] §15.4.8 完全サンプル JSON（マスク済）
- [ ] §15.5 プロンプト雛形を本文に含む（コピペ可能）
- [ ] §15.5.4 OPSEC 規律（本番 API キー禁止等）
- [ ] §15.6 提案 JSON の許容キー / 禁止キー対比表
- [ ] §15.6.4 部分 Apply 禁止の根拠 3 項目
- [ ] §15.7 差分プレビュー JSON 例、Apply 活性条件
- [ ] §15.7.3 wait_reason / リスクパラメータは触れない明記
- [ ] §15.8 Apply シーケンス Mermaid + 3 段バリデーション
- [ ] §15.8.4 `strategy_versions` / `daily_reports` / `audit_log` / FS 書込内容
- [ ] §15.8.5 ロールバック 24 時間規定
- [ ] §15.8.6 トランザクション失敗マトリクス
- [ ] §15.9 v1.1 検討の明記
- [ ] §15.10 失敗マトリクス + `E_NIGHTLY_*` 候補列挙
- [ ] §15.10.3 緊急停止が `NIGHTLY_REVIEW` を上書き可能
- [ ] §15.11 相互参照表（ch3, 8, 9, 10, 11, 12, 17, 18, 20）
- [ ] Mermaid フェンス全て閉じている
- [ ] 用語（`NIGHTLY_REVIEW`, `applied_by`, `wait_reason` 等）が ch10 と一致

### 15.12.2 一次レビュー観点（7 項目）

マスター用、各項目を質問形式で:

1. **整合性**: §15.3.2 で採用した `NIGHTLY_REVIEW` 単一状態は ch3 §3.1 の細分 3 状態（`GENERATING_REPORT` / `AWAITING_APPLY` / `APPLYING_STRATEGY`）とどう統合するか？ ch3 を更新するか、ch3 をサブフェーズとして残すかを M1.4 中に確定できるか？
2. **データ契約**: §15.4 の `summary_json` スキーマは PHASE 3 実装で `NightlyReporter.generate()` から迷いなく出力できるか。`balance_start_usd` の取得元（前日終端 `bot_state.balance` か `trades` 集計か）は明記されているか？
3. **LLM 連携**: §15.5.2 のプロンプト雛形は Opus 4.7 が `rationale` 含む JSON のみを返すよう十分制約しているか。説明文混入の検出は UI 側 `JSON.parse` で過不足ないか？
4. **Apply 安全性**: §15.7.4 の活性条件（±20% で拒否、±10% は警告のみ活性）は、実運用で Opus が「学習が進んで急変を提案する」ケースを過度にブロックしないか。±20% 強制承認パスの不在は妥当か？
5. **トランザクション**: §15.8.6 の「DB COMMIT 済 / `strategy.json` 書込失敗」のリカバリ（起動時整合性チェック）は ch20 / 起動シーケンス（ch3 / ch6）で具体化される予定か？ ch20 着手時に本章へ後方リンクを足せるか？
6. **エラーコード**: §15.10.1 の `E_NIGHTLY_001` 〜 `E_NIGHTLY_014` は 14 個に細分されているが、ch18 確定時に `E_DB_*` / `E_STATE_*` への統合で過半数が消える想定か。命名空間の責任境界（`E_NIGHTLY_*` を残すべきは何件か）は妥当か？
7. **章間役割**: §15.2.2 で「UI=ch8 / フロー=ch9 / データ・タイミング=本章」と分担したが、ch8 §8.14.5 の差分検証ロジックと §15.7 の差分仕様で書込内容に重複がある。重複部分は ch8 を参照に置き換えるべきか、両方が SSOT 的に共存するか？

### 15.12.3 既知の未確定事項

| 項目 | 状態 | 確定先 |
|------|------|--------|
| ch3 細分状態と本章 `NIGHTLY_REVIEW` の統合 | `[要確認: ch3]` | M1.4 レビュー |
