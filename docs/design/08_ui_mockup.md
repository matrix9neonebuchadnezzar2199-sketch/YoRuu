...
**承認日**: 2026-05-27  
**ステータス**: APPROVED  
**最終更新**: 2026-05-28（v1.2.3: §8.4.3 severity パレット、§8.7.4 緊急停止色制限、ch18 連携）  
...
:root {
  ...
  --header-height: 48px;
  --banner-height: 36px;
  /* セマンティック背景（半透明、hover・active・バナー背景用） */
  --severity-info-bg:    #1f6feb1a;
  --severity-success-bg: #3fb9501a;
  --severity-warn-bg:    #d299221a;
  --severity-danger-bg:  #f851491a;
}
```
...
### 8.4.3 Severity セマンティックトークン（ch18 連携）

アラート・夜間レビュー差分・緊急停止周辺の背景色は **ch18 §18.3 の `severity` 列**に従い、以下の CSS 変数のみを使用する（新規色の直書き禁止）。

| ch18 `severity` | 前景トークン | 背景トークン | 主な用途 |
|-----------------|-------------|-------------|----------|
| `ERROR` / `CRITICAL` | `--danger-fg` | `--severity-danger-bg` | Apply 拒否、不変条件違反、緊急停止 FAB |
| `WARN` | `--attention-fg` | `--severity-warn-bg` | `E_NIGHTLY_008` 10%超警告、ヘルス劣化バナー |
| （将来）`INFO` | `--text-link` | `--severity-info-bg` | 情報バナー（v1.2.3 未使用） |
| 成功系 | `--success-fg` | `--severity-success-bg` | 復旧・Apply 成功トースト |

§8.7.4 のコマンドパレット緊急停止行も `ERROR` 行として `--danger-fg` / `--severity-danger-bg` に限定する。
...
### 8.7.4 緊急停止と severity 色制限

緊急停止 UI（サイドバー FAB §8.6.3、コマンドパレット項目、§8.10 画面）は **常に ch18 `severity=ERROR` 相当**の配色に固定する。

| UI 要素 | 許可トークン | 禁止 |
|---------|-------------|------|
| サイドバー FAB ラベル | `--danger-fg` | `--attention-fg`（WARN 黄） |
| FAB / パレット行ホバー背景 | `--severity-danger-bg` | `--severity-warn-bg` |
| コマンドパレット「緊急停止」行 | 上記 ERROR 行のみ | 成功・警告色での強調 |

`WARN` 系（`--attention-fg` / `--severity-warn-bg`）は夜間レビュー差分（ch15 §15.7.2、`E_NIGHTLY_008` 10%超）やアラート一覧（§8.16）に限定し、緊急停止と混同しない。

コマンドマッチ: 完全一致 `緊急停止` / `emergency stop` のみ。`stop`・`halt` 等の短縮一致は誤発火防止のため不可。
...
日次レポート確認、外部 AI 出力 JSON のペースト、差分確認、Apply（検証 → 差分 → 確認ダイアログ → 適用）。
...
- [ ] 言語切替で ja → en（en 欠落キーは §14.5 の `ja` フォールバック）→ ja に戻ることを確認
...
- [x] §8.1 〜 §8.26 がすべて存在
- [x] 全11画面 + ハブの仕様が揃っている
- [x] §8.4 のカラー変数が `shared/style.css` にそのまま貼り付け可能（v1.2.1 で severity 背景変数を追記済み）
- [x] §8.9 の SSE イベントが第10章で再利用できる詳細度
- [x] §8.25 の引き継ぎが PHASE 2 着手者に十分
- [x] 章間相互参照が新章番号と整合
- [x] cross-ref 表に重複・抜け漏れがない（第9章は M1.3 執筆後に §8.26.2 へ追記可）
